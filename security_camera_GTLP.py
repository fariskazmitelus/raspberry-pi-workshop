...
DISTANCE_THRESHOLD = 100  # Try starting with 100!
...
# Optional variables for different distances
DANGER_ZONE = 50
WARNING_ZONE = 100
SAFE_ZONE = 200
...
def create_security_message(distance):
    ...
    Examples:
    return f"ALERT! Person detected {distance:.1f} cm away!"
...
def main():
    ...
    try:
        while True:
            #1: Get the distance from the ultrasonic sensor
            ...
            #3: Check if something is closer than DISTANCE_THRESHOLD
            # HINT: Use an if statement with distance and DISTANCE_THRESHOLD
            if distance < DISTANCE_THRESHOLD:            
                #4: Check if a person was detected
                person_detected = InferenceTensorFlow(grey, interpreter, labels)
            
                #5: If a person is detected and they're too close, create an alert!
                if person_detected:
                    # Update detection statistics (these can be optional)
                    detection_count += 1
                    ...
                    alert = create_security_message(distance)
                    zone_status = check_security_zone(distance)
                    
                    # Print formatted alert (can change based on what students want to present as)
                    print("\n" + "="*50)
                    print(alert)
                    print(zone_status)
                    print(f"Total Detections: {detection_count}")
                    print(f"Time since last detection: {time_since_last:.1f} seconds")
                    print("="*50 + "\n")
